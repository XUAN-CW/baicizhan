...
//    public static final String cloneUrl = "https://github.com/XUAN-CW/staticPage";

    public static File getBaicizhanResourceRootDir() {
        return baicizhanResourceRootDir;
    }
    ...
    public static List<File> getWordResourceDirList() {
        if(PathUtil.getWordResourceRootDir().exists()){
            File[] wordResourceDirArray = PathUtil.getWordResourceRootDir().listFiles();
            if(ArrayUtils.isNotEmpty(wordResourceDirArray)){
                return Arrays.stream(wordResourceDirArray).collect(Collectors.toList());
            }
        }
